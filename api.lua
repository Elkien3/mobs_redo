...
local smart_mobs = function(self, s, p, dist, dtime)
	...
	local has_lineofsight = minetest.line_of_sight({x=s.x, y=(s.y)+.5, z=s.z}, {x=target_pos.x,y=(target_pos.y)+1.5,z=target_pos.z}, .2)
	...
	if not has_lineofsight then
		if los_switcher == true then
			use_pathfind = true
			los_switcher = false
			--minetest.chat_send_all("cannot see target!")
		end
	else
		if los_switcher == false then
			...
			--minetest.chat_send_all("can see target!")
		end
	end
...
--		self.path.way = minetest.find_path(s, p1, 16, 2, 6, "Dijkstra")
		...
		--minetest.chat_send_all("pathfind!")
		
		--[[if self.path.way and table.getn(self.path.way) > 0 then
